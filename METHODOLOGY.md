...
- Для переноса строки внутри подписи узла или метки перехода используйте `\n` — транспилятор автоматически преобразует его в `<br/>` для корректного рендеринга
...
**Перенос строки в подписях узлов и меток:**

````markdown
```mermaid Граф переходов состояний
stateDiagram-v2
    [*] --> A : Начало\n(инициализация)
    A --> B : Событие 1\nусловие выполнено
```
