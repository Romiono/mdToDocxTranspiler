...
- В приложениях: `Рисунок А.1 – Описание`
- Перенос строки внутри подписи узла или метки перехода — `\n`: транспилятор автоматически преобразует его в `<br/>` при рендеринге
...
Диаграмма состояний с многострочными метками (`\n` → перенос строки):
````markdown
```mermaid Граф переходов состояний процесса
stateDiagram-v2
    [*] --> СОЗДАН : Инициализация\n(системой)
    СОЗДАН --> АКТИВЕН : Подтверждение\nпользователем
    АКТИВЕН --> ЗАВЕРШЁН : Успешное\nвыполнение
    АКТИВЕН --> ОТМЕНЁН : Ошибка\nили отказ
    ЗАВЕРШЁН --> [*]
    ОТМЕНЁН --> [*]
```
````
